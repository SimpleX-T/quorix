...
const bot = new Bot("7582138457:AAEIh0EvVkQqR4o8QYIDgj-r8DXkIi79T3A");
...
bot.command("comedm", async (ctx) => {
	if (ctx.config.isGroupChat) {
		...
		await ctx.reply(
			"We're already in a private chat! How can I help you?",
			{
				reply_to_message_id: ctx.message.message_id,
			}
		);
...
bot.command("activegames", async (ctx) => {
	if (!ctx.config.isDM) {
		return await ctx.reply(
			"Please check active games in a private message!",
			{
				reply_to_message_id: ctx.message.message_id,
			}
		);
...
bot.command("endgame", async (ctx) => {
	...
	if (existingGame) {
		...
		await ctx.reply(
			"The current game has been ended by an administrator.",
			{
				reply_to_message_id: ctx.message.message_id,
			}
		);
